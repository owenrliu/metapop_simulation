...
Age-structured matrix model, where population is structured into $\omega$ age classes, and abundance denoted by $\textbf{N} = (N_{1},...,N_{\omega})$. A vector $\textbf{p}$ describes survival, where, for example, $p_{i}$ is the proportional survival of individuals from age $i$ to age $i+1$, subject to fishing and natural mortality.  Similarly, $\textbf{f}_{i}$ is the fertility of individuals of age $i$.  $\xi$ is the number of external larvae supplied to a local population; $a \in [0,1]$ is the self-recruitment rate, that is, the proportion of locally produced larvae that return to that population; and $\gamma$ is the proportion of settlers that remain alive at the end of the first year.
...
#### Selectivity-at-age
Selectivity at age is calculated by first converting age to length, then using a logistic relationship $W=aL^{b}$ to calculate a vector of weights.  Weight-at-age is used to calculate spawning stock biomass and biomass yield for fisheries
...
#### Fisheries Yield
Yield at each time step will be equal to the exploitation fraction(the proportion of fishing mortality relative to total mortality), times the overall survival rate, times weight-at-age and numbers-at-age, and summed over all age classes. The output will be a total catch in kg, subject to fishery selectivity and all of the demographic parameters and relationships defined above.  This equation will require the outputs of most of the equations above.
...
sim.dipop <- function(nyears=200,max.age=14,Linf=52.2,K=0.354,t0 = -0.766, surv=0.863, mort=1-0.863, repr.age=2, gamma=0.125,fert1=0.0129,fert2=3.03, fish=0, L50=25,L95=40, lw.a=0.0105, lw.b=3.11, a.mu=0.0016,a.cv=0.5, ext.mean=1000, ext.cv=0.5) {
  ## age vector
  age.vec <- 0:max.age
  ...
  ## fertility vector
  f.vec <- fert(age.vec=age.vec,vb=vb,repr.age=repr.age,fert1=fert1,fert2=fert2)
