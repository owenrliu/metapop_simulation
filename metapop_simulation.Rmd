...
```{r local d.i. population simulation 1, echo=FALSE}
d.i.sim1 <- matrix(nrow=(max.age+1), ncol=200)
d.i.simtot <- numeric()
Ninit <- rep(250,15)
d.i.sim1[,1] <- Ninit
d.i.simtot <- sum(Ninit)
for (i in 1:199) {
  a.rand <- rbeta(n=1,shape1=0.01,shape2=10)
  e.rand <- rnorm(1,mean=ext,sd=(0.5*ext)) #with somewhat stochastic external larval supply
  M <- M.di(a=a.rand, gamma=gamma, f.vec=f.vec, p.vec=p.vec)
  N.vec <- M%*%d.i.sim1[,i] + (gamma*e.rand*e.vec)
  d.i.sim1[,i+1] <- N.vec
  d.i.simtot[i+1] <- sum(N.vec)
  }
plot(1:200,d.i.sim1[15,],type='l')
```