...
sim.dipop <- function(nyears=200,max.age=14,Linf=52.2,K=0.354,t0 = -0.766, surv=0.863, repr.age=2, gamma=0.125,
                    f1=0.0129,f2=3.03,a.mu=0.001,a.cv=0.5, ext.mean=1000, ext.cv=0.5) {
  ## age vector
  age.vec <- 0:max.age
  ...
  a.rand <- a.rand <- rbeta(n=nyears,shape1=a.alpha,shape2=a.beta)
  ...
  Ninit <- rep(250,15) # starting abundance (equal in all classes)
  d.i.sim[,1] <- Ninit
  ...
  demo.rand <- runif(1,min=0.875,max=1.125) # allows uniform random synchronous variation
  M <- M.di(a=a.rand[nyears], gamma=gamma*demo.rand, f.vec=f.vec*demo.rand, p.vec=p.vec*demo.rand)
...
sim.ddpop <- function(nyears=200,max.age=14,Linf=52.2,K=0.354,t0 = -0.766, surv=0.863, repr.age=2, gamma=0.125,
                    f1=0.0129,f2=3.03,a.mu=0.001,a.cv=0.5, ext.mean=1000, ext.cv=0.5, B=0.0001) {
  ## age vector
  age.vec <- 0:max.age
  ...
  for (i in 1:(nyears-1)) {
    ...
    M <- M.dd(max.age=max.age,a=a.rand[i], gamma=gamma*demo.rand, f.vec=f.vec*demo.rand,
              p.vec=p.vec*demo.rand,n.vec=d.d.sim[,i],ext=ext.mean, B=B*demo.rand)
    eigens[i] <- eigen(M)[[1]][1] #store the dominant eigen value
    Radj[i] <- net.R(f.vec=f.vec*demo.rand,p.vec=p.vec*demo.rand)*(gamma*demo.rand)*a.rand[i]
    N.vec <- M%*%d.d.sim[,i] + (gamma*ext.rand[i]*e.vec)
    d.d.sim[,i+1] <- N.vec
