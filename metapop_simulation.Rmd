...
```{r d.i. local population simulation function, echo=FALSE, include=FALSE}

## Calls all the parts we've coded so far
# sim.pop <- function(nsims=100,nyears=200,max.age=14,Linf=52.2,K=0.354,t0 = -0.766, surv=0.863, repr.age=2, f1=0.0129,f2=3.03,a.alpha=4,a.beta=4000,gamma, f.vec, p.vec,n.vec,ext, B) {
  age.vec <- 0:max.age
  l.vec <- vb.growth(age.vec=0:max.age,vb=vb) #vector of lengths (to be used later)
  f.vec <- fert(age.vec=0:max.age,vb=vb,repr.age=repr.age,f1=f1,f2=f2)
...
mean.props <- apply(sim1.props, MARGIN=1, FUN=mean) #mean proportions
sd.props <- apply(sim1.props, MARGIN=1, FUN=sd) #sd proportions
cv.props <- apply(sim1.props, MARGIN=1, FUN= function(x) sd(x)/mean(x)) #cv of proportions
```